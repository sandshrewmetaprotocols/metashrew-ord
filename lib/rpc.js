...
class MetashrewOrd {
    ...
    async satranges({ outpoint }) {
        ...
        const buffer = ethers_1.ethers.hexlify(ordinals.SatRangesRequest.toBinary({
            outpoint: {
                hash: ethers_1.ethers.toBeArray(addHexPrefix(reversed)),
                vout: Number(vout)
            }
        }));
    ...
    async sat({ sat }) {
        const buffer = ethers_1.ethers.hexlify(ordinals.SatRequest.toBinary({
            sat
        }));
        const byteString = await this._call({
            method: 'sat',
        ...
        return {
            ...
            outpoint: {
                txid: stripHexPrefix(ethers_1.ethers.hexlify(decoded.outpoint.hash)),
